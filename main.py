...
TFIDF_MATRIX_PATH = os.path.join(BASE_DIR, "Models\tfidf_matrix.pkl")
TFIDF_PATH = os.path.join(BASE_DIR, "Models\tfidf.pkl")
...
async def tmdb_cards_from_results(
    ...
    for m in (results or [])[:limit]:
        out.append(
            TMDBMovieCard(
                tmdb_id=int(m["id"]),
                title=m.get("original_title") or m.get("name") or "",
...
def tfidf_recommend_titles(
    ...
    for i in order:
        if int(i) == int(idx):
            continue
        try:
            title_i = str(df.iloc[int(i)]["original_title"])
        except Exception:
            continue
...
async def attach_tmdb_card_by_title(title: str) -> Optional[TMDBMovieCard]:
    ...
    try:
        ...
        return TMDBMovieCard(
            tmdb_id=int(m["id"]),
            title=m.get("original_title") or title,
            poster_url=make_img_url(m.get("poster_path")),
            release_date=m.get("release_date"),
            vote_average=m.get("vote_average"),
        )
...
def load_pickles():
    ...
    # sanity
    if df is None or "original_title" not in df.columns:
